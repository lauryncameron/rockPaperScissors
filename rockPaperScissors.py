...
Second, we will create coded that takes the input (choice) from user
Third, we will create code that takes the 'input' (choices) from the computer
Fourth, we will create code that check if the user wants to quit, or if the user doesn't enter one of the options (rock, paper, scissors)
Fifth, we will loop each round of the game
Sixth, we will add statements at the end of the beginning that welcome and thank the user playing
...
while(keepPlaying):
    ...
    while(humanScore < 2 and cpuScore < 2):
        ...
        if (choice.lower() == 'q'):
            ...
            print("Human: " + humanScore.__str__() + " CPU:" +
        cpuScore.__str__())
